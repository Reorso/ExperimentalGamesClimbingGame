...
public class ClimbController2 : MonoBehaviour
{
    //float lenght;
    ...
    private void Update()
    {
        for (int i = 0; i < 4; i++)
        {
            if (locked[i])
            {
                limbs[i].data.target.position -= Vector3.up * 0.1f * Time.deltaTime;
        ...
        {
            centreC.weight = 1;
            centre.position -= Vector3.up * 0.1f * Time.deltaTime;
            centroid.transform.position -= Vector3.up * 0.1f * Time.deltaTime;
            s = false;
        }
